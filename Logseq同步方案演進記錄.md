      ...
      - **關鍵改進**：
        ...
        5. **持久的 SSH 認證** ✅：鑰匙串集成確保重啟後認證有效
          <br>
            ...
            🔍 **深入分析** ： <br> 看似相同的工具（fswatch），但通過精確控制和完善的錯誤處理，實現了完全不同的結果。
...
- ## 7.1 近期真實排查與優化歷程
  ...
  - ### Logseq 內建 Git 功能與同步腳本的衝突
    1. 測試發現，只要 Logseq 的「Git auto commit」或相關插件開啟，無論同步腳本多麼靜默，Logseq 仍會在遇到 git lock ref 等錯誤時強制彈窗，干擾使用體驗。
    2. 經過多輪測試，最終選擇**完全關閉 Logseq 內建 Git 功能與插件**，只保留 logseq_sync.sh 腳本負責所有同步，徹底杜絕彈窗。
    <br>
