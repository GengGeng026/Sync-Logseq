...
while true; do
    ...
    # 1. 保存本地修改
    echo "[$(date)] 檢查本地修改（包括腳本自身）" >> $LOG_FILE
    
    # 強制將腳本自身加入追蹤
    git add auto_sync.sh -f >> $LOG_FILE 2>&1
    ...
    if ! git diff --staged --quiet; then
        echo "[$(date)] 發現本地修改，立即提交" >> $LOG_FILE
        git commit -m "自動保存: $(date)" >> $LOG_FILE 2>&1
        
        # 立即推送（不等待）
        echo "[$(date)] 立即推送變更到遠程" >> $LOG_FILE
        git push origin main >> $LOG_FILE 2>&1
        echo "[$(date)] 推送完成" >> $LOG_FILE
    else
        echo "[$(date)] 無本地修改需要提交" >> $LOG_FILE
    fi
